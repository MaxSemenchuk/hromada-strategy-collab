...
_ADJ = "(?:ська|зька|цька|ька|ської|зької|цької|ької)"
...
# The adjective itself keeps its -ська/-цька ending so partner_stem can
# strip via adj_stem ("Шацька" → "шац", not the non-greedy "ша").
COUNCIL_ADJ = re.compile(
    r"([А-ЯІЇЄҐ][а-яіїєґ'’\-]+" + _ADJ + r")\s+"
    r"(?:сільськ\w*|селищн\w*|міськ\w*)\s+"
    r"(?:рад\w*|військов\w*\s+адміністрац\w*|територіальн\w*\s+громад\w*)",
    re.I,
)
# «Новояворівська територіальна громада» — no міська/селищна/сільська in the string.
HROMADA_ONLY = re.compile(
    r"([А-ЯІЇЄҐ][а-яіїєґ'’\-]+" + _ADJ + r")\s+"
    r"територіальн\w*\s+громад",
    re.I,
)

OBLAST_STEM = re.compile(
    r"(вінницьк|волинськ|дніпропетровськ|донецьк|житомирськ|закарпатськ|"
    r"запорізьк|івано-франківськ|київськ|кіровоградськ|луганськ|львівськ|"
    r"миколаївськ|одеськ|полтавськ|рівненськ|сумськ|тернопільськ|"
    r"харківськ|херсонськ|хмельницьк|черкаськ|чернівецьк|чернігівськ)",
    re.I,
)
