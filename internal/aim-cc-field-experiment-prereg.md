...
**Corpus ceiling resolved in v0.7** (see §12): with the sampling frame widened from 293 to **1,424** hromadas (every hromada with a KATOTTG — `recommend_for_no_goals()` now covers the rest), the max-per-arm no-reuse ceiling rises from 146 to **712**. Re-run 2026-09-04 (`aim_cc_synthetic_experiment.py --design report --guaranteed-view --totals "250,292,600,1000,1424"`, seed 42, 3,000 MC reps/point) against the widened frame, same illustrative priors as before (§11) — full table in `internal/aim-cc-synthetic-experiment-results.json`:

| Plan | Hromadas/arm | Total | Power (inbound@30d) | Notes |
|------|--------------|-------|----------------------|-------|
| **Pilot** | 125 | 250 | **33.1%** | enough for consultation + a directional rate |
| **Full 293-corpus (old reference point)** | 146 | 292 | **39.2%** | was the hard ceiling pre-v0.7; kept as a reference row, no longer an actual limit |
| **Adequately powered** | 300 | 600 | **80.9%** | now achievable within the existing 1,424-hromada pool, **no new data collection needed** — previously flagged as needing corpus growth |
| **High power** | 500 | 1,000 | **97.0%** | |
| **Full widened pool (new ceiling)** | 712 | 1,424 | **99.4%** | max achievable without relaxing the no-reuse rule |

This remains an **underpowered pilot** for small effects at the "Pilot" row only; at 300/arm — now reachable without growing the corpus — power is adequate at the current illustrative priors. Real priors will differ from these elicited placeholders (§11), so treat the specific percentages as directional, not a guarantee; re-run again once any real funnel data exists. Formal launch decision should still budget for a smaller-than-illustrative true effect size (adaptive stop only for logistics, not peeking on H1).
